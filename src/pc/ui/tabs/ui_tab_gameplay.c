...
void UI_Tab_Gameplay(struct nk_context *ctx) {
    ...
    if (nk_button_label(ctx, "Stop Music")) Music_Init();

    UI_Header(ctx, "Time Attack & Replays");
    nk_layout_row_dynamic(ctx, 30, 3);

    if (gGhost.recording) {
        if (nk_button_label(ctx, "Stop Recording")) Ghost_StopRecording();
    } else {
        if (nk_button_label(ctx, "Record Ghost")) Ghost_StartRecording();
    }

    if (gGhost.playing) {
        if (nk_button_label(ctx, "Stop Playback")) gGhost.playing = false;
    } else {
        if (nk_button_label(ctx, "Play Ghost")) Ghost_StartPlayback();
    }

    if (nk_button_label(ctx, "Save Ghost")) Ghost_Save("ghost.dat");
    if (nk_button_label(ctx, "Load Ghost")) Ghost_Load("ghost.dat");
    ...
    UI_Checkbox(ctx, "Enable Side Attack", &side_attack, "Double tap L or R to lunge laterally and damage enemies.");
    UI_Checkbox(ctx, "Enable Spin Attack", &spin_attack, "Press Z to spin rapidly, damaging enemies in a radius.");
