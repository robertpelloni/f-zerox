...
void UI_Tab_Visuals(struct nk_context *ctx) {
    ...
    UI_Checkbox(ctx, "Fullscreen", &fullscreen, "Toggle Borderless Windowed Fullscreen.");
    gConfig.fullscreen = fullscreen;

    int filtering = gConfig.bilinear_filtering;
    UI_Checkbox(ctx, "Bilinear Texture Filtering", &filtering, "Smooths out pixelated textures (N64 style vs Crisp).");
    gConfig.bilinear_filtering = filtering;
